...
tags:
  ...
  - 服务端API
---
![[development-official-authorized-preview-approval-attachment_O1CN01Zx4PED22rkUkmRe1L_!!6000000007174-2-tps-226-40.png]]![[development-official-authorized-preview-approval-attachment_O1CN01UwOv1C27lrTuGFj2D_!!6000000007838-2-tps-17-30.png]]

更新于 2025-12-08调用本接口，授权预览审批附件。
...
2. 根据审批钉盘空间spaceId，网页应用（H5微应用）/小程序，通过[上传附件到钉盘/从钉盘选择文件](/document/development/jsapi-upload-attachment-to-ding-talk)获取钉盘附件file的信息。
...
5. 调用**预览钉盘文件**接口，进行预览，调用[预览钉盘文件](/document/development/jsapi-preview-file-in-ding-talk)接口。
...
| 企业内部应用 | 支持 | 工作流实例写权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=org&api=workflow_1.0%23GetSpaceWithDownloadAuth) |
| 第三方企业应用 | 支持 | 工作流实例写权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=isv&api=workflow_1.0%23GetSpaceWithDownloadAuth) |
...
| agentId | Long | 否 | 应用的agentId：   * 企业内部应用，可查看[AgentId](/document/development/development-basic-concepts#884d363067bnq)信息。 * 第三方企业应用，可调用[获取企业授权信息](/document/development/obtains-the-basic-information-of-an-enterprise)接口获取`agentid`参数值。 |
...
| fileId | String | 是 | 审批附件ID。  **说明**  fileId必须与发起审批实例中附件组件中的文件fileId保持一致，否则出现无权限错误信息。 |
