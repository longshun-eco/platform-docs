...
tags:
  ...
  - 服务端API
---
![[development-api-previewpublishedprocess_O1CN01Zx4PED22rkUkmRe1L_!!6000000007174-2-tps-226-40.png]]![[development-api-previewpublishedprocess_O1CN01UwOv1C27lrTuGFj2D_!!6000000007838-2-tps-17-30.png]]

更新于 2025-09-23发起流程前，预览流程流向。预览流程发起后的审批人情况，不会真正发起流程，不产生业务数据。
...
| 企业内部应用 | 支持 | 宜搭流程数据读权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=org&api=yida_1.0%23PreviewPublishedProcess) |
| 第三方企业应用 | 支持 | 宜搭流程数据读权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=isv&api=yida_1.0%23PreviewPublishedProcess) |
...
| --- | --- | --- | --- |
| 名称 | 类型 | 是否必填 | 描述 |
| appType | String | 是 | 应用编码。 ![[development-api-previewpublishedprocess_O1CN01auMafb1gfuQBoESlW_!!6000000004170-2-tps-2694-1114.png]] |
| systemToken | String | 是 | 应用密钥，在应用数据中获取。 ![[development-api-previewpublishedprocess_O1CN01jA8xVe1ZStrN1fmGj_!!6000000003194-2-tps-2698-1144.png]] <props="intl">只发布虚商站 |
| userId | String | 是 | 用户的userid。 |
| language | String | 否 | 语言，取值：   * zh\_CN：中文（默认值） * en\_US：英文 |
| formUuid | String | 是 | 表单ID。 ![[development-api-previewpublishedprocess_O1CN01HdY3Dp1PuFLl6yIFS_!!6000000001900-2-tps-2688-1160.png]] |
| formDataJson | String | 是 | 表单数据，示例：`"{\"textField_jcpm6agt\": \"单行\",\"employeeField_jcos0sar\": [\"workno\"]}"`   * key：流程组件标识，宜搭表单编辑页面，高级设置中查看。 * value：流程组件内的值。 ![[development-api-previewpublishedprocess_O1CN01yTqqI51eViHNpsmb2_!!6000000003877-2-tps-2662-1014.png]] |
| processCode | String | 否 | 流程编码。 ![[development-api-previewpublishedprocess_O1CN01wfayc31oU13xpzPfX_!!6000000005227-2-tps-2704-1152.png]] |
...
| operatorUserId | String | 操作者userId。  **说明**   * 对于普通节点，返回操作人useriId。 * 对于发起人自选节点，该字段会返回发起人自选节点唯一标识，例如：**\_\_optionalApproval\_node\_ocltdztr2b1**。 |
| activityId | String | 节点ID。 |
| taskType | String | 任务类型：   * 一个一个分发 COMMON\_ONE\_BY\_ONE * 一次性分发COMMON\_ALL\_AT\_ONCE * 中间插入的任务类型（一个一个分发）MIDDLE\_IN\_ONE\_BY\_ONE * 中间插入的任务类型（一次性分发）MIDDLE\_IN\_ALL\_AT\_ONCE   **说明**  只有运行的待办任务才会都有值。 |
| taskExecuteType | String | 任务执行类型：   * NORMAL: 正常的执行 * FILTERED: 过滤的执行 * AUTO: 自动审批的执行   **说明**  只有运行的待办任务才会有值。 |
| size | Integer | 大小。 |
