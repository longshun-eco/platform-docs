...
tags:
  ...
  - 服务端API
---
![[development-query-the-application-list_O1CN01Zx4PED22rkUkmRe1L_!!6000000007174-2-tps-226-40.png]]![[development-query-the-application-list_O1CN01UwOv1C27lrTuGFj2D_!!6000000007838-2-tps-17-30.png]]

更新于 2025-10-09用于查询组织下的宜搭应用列表，包括宜搭应用状态、宜搭应用图标和宜搭应用编码等。

例如，宜搭应用如下图所示，调用本接口获取企业内宜搭应用详情信息。

![[development-query-the-application-list_O1CN0194vG2e1LrGEWNSBDO_!!6000000001352-2-tps-2422-928.png]]
...
| 企业内部应用 | 支持 | 宜搭应用数据读权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=org&api=yida_1.0%23ListApplication) |
| 第三方企业应用 | 支持 | 宜搭应用数据读权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=isv&api=yida_1.0%23ListApplication) |
| 第三方个人应用 | 暂不支持 | 宜搭应用数据读权限 | 暂不支持 |

## 请求方法

```
GET /v1.0/yida/organizations/applications?appFilter=String&pageNumber=Integer&corpId=String&pageSize=Integer&appNameSearchKeyword=String&userId=String&token=String HTTP/1.1
...
| token | String | 是 | 根据corpId、userId和CorpToken使用md5加密计算生成的字符串。  **说明**  该参数需线下提供，请通过宜搭技术支持咨询。 |
...
| name | String | 宜搭应用的名称。 |
| systemToken | String | 应用密钥。  **说明**  仅宜搭专属版。 |
