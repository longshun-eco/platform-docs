...
tags:
  ...
  - 服务端API
---
![[development-batch-update-of-component-values-in-form-instances_O1CN01Zx4PED22rkUkmRe1L_!!6000000007174-2-tps-226-40.png]]![[development-batch-update-of-component-values-in-form-instances_O1CN01UwOv1C27lrTuGFj2D_!!6000000007838-2-tps-17-30.png]]

更新于 2026-03-16根据宜搭表单实例Id，批量更新宜搭表单实例的组件值。

例如，企业宜搭应用的表单实例列表如下图，调用本接口批量更新四个表单内**报销项目**组件值。将**报销项目**的值修改为**更新报销项目**。

![[development-batch-update-of-component-values-in-form-instances_O1CN01Dgf26D29tlBNIfxDH_!!6000000008126-2-tps-2392-1470.png]]

接口调用成功后，宜搭应用的表单实例列表如下：

![[development-batch-update-of-component-values-in-form-instances_O1CN01EzBmF41U4YekHD8zj_!!6000000002464-2-tps-2380-1440.png]]
...
| --- | --- | --- | --- |
| 企业内部应用 | 支持 | 宜搭表单数据写权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=org&api=yida_1.0%23BatchUpdateFormDataByInstanceId) |
| 第三方企业应用 | 支持 | 宜搭表单数据写权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=isv&api=yida_1.0%23BatchUpdateFormDataByInstanceId) |
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| noExecuteExpression | Boolean | 否 | 是否不触发表单绑定的校验规则、关联业务规则和第三方服务回调。   * true：不触发 * false：触发   **说明**  该参数值传true可以降低API的耗时以及获得更大的单次更新数据量上限。 |
| formUuid | String | 是 | 表单编码。  **说明**  该参数从宜搭应用中获取。 ![[development-batch-update-of-component-values-in-form-instances_O1CN01JtSC2M20A7ARy6uY9_!!6000000006808-2-tps-2326-866.png]] |
| updateFormDataJson | String | 是 | 用于更新表单实例的数据，格式参考[创建或更新表单数据格式说明](https://open.dingtalk.com/document/development/instructions-for-creating-or-updating-form-data-format)。 |
| appType | String | 是 | 宜搭应用编码。  **说明**  该参数从宜搭应用中获取。 ![[development-batch-update-of-component-values-in-form-instances_O1CN01JtSC2M20A7ARy6uY9_!!6000000006808-2-tps-2326-866.png]] |
| ignoreEmpty | Boolean | 否 | 是否忽略空值。   * true：忽略 * false：不忽略 |
| systemToken | String | 是 | 宜搭应用密钥。  **说明**  该参数从宜搭应用中获取。 ![[development-batch-update-of-component-values-in-form-instances_O1CN01JtSC2M20A7ARy6uY9_!!6000000006808-2-tps-2326-866.png]] |
| useLatestFormSchemaVersion | Boolean | 否 | 是否使用最新的表单schema版本。   * true：是 * false：否，默认值 |
| asynchronousExecution | Boolean | 否 | 是否需要宜搭服务端异步执行该任务。   * true：需要 * false：不需要 |
| formInstanceIdList | Array of String | 是 | 表单实例Id。   * 企业内部应用，调用[获取实例ID列表](https://open.dingtalk.com/document/orgapp/obtains-a-list-of-instance-ids)接口或者[获取多个表单实例ID](https://open.dingtalk.com/document/orgapp/obtain-the-ids-of-multiple-form-instances)接口获取。 * 第三方企业应用，调用[获取实例ID列表](https://open.dingtalk.com/document/isvapp/obtains-a-list-of-instance-ids)接口或者[获取多个表单实例ID](https://open.dingtalk.com/document/isvapp/obtain-the-ids-of-multiple-form-instances)接口获取。 |
