...
tags:
  ...
  - 服务端API
---
![[development-create-multiple-form-instances_O1CN01Zx4PED22rkUkmRe1L_!!6000000007174-2-tps-226-40.png]]![[development-create-multiple-form-instances_O1CN01UwOv1C27lrTuGFj2D_!!6000000007838-2-tps-17-30.png]]

更新于 2026-03-16用于批量创建表单实例数据，返回新增表单实例的Id。

例如，企业宜搭应用的数据管理中宜搭实例列表如下图所示，调用本接口在宜搭实例列表中新增一个报销项目为“批量新增表单实例”的实例。

![[development-create-multiple-form-instances_O1CN01qBwuXB1MSMJSnNhW1_!!6000000001433-2-tps-2404-1460.png]]

调用成功后，宜搭应用的数据管理列表中可查看新增的实例。

![[development-create-multiple-form-instances_O1CN01Dgf26D29tlBNIfxDH_!!6000000008126-2-tps-2392-1470.png]]
...
| --- | --- | --- | --- |
| 企业内部应用 | 支持 | 宜搭表单数据写权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=org&api=yida_1.0%23BatchSaveFormData) |
| 第三方企业应用 | 支持 | 宜搭表单数据写权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=isv&api=yida_1.0%23BatchSaveFormData) |
...
| 名称 | 类型 | 是否必填 | 描述 |
| --- | --- | --- | --- |
| noExecuteExpression | Boolean | 否 | 是否不触发表单绑定的校验规则、关联业务规则和第三方服务回调。   * true：不触发 * false：触发   **说明**  该参数值传true可以降低API的耗时以及获得更大的单次保存数据量上限。 |
| formUuid | String | 是 | 表单编码。  **说明**  该参数从宜搭应用中获取。 ![[development-create-multiple-form-instances_O1CN01JtSC2M20A7ARy6uY9_!!6000000006808-2-tps-2326-866.png]] |
| appType | String | 是 | 宜搭应用编码。  **说明**  该参数从宜搭应用中获取。 ![[development-create-multiple-form-instances_O1CN01JtSC2M20A7ARy6uY9_!!6000000006808-2-tps-2326-866.png]] |
| asynchronousExecution | Boolean | 否 | 是否需要宜搭服务端异步执行该任务。   * true：允许。 * false：不允许。 |
| systemToken | String | 是 | 宜搭应用密钥。  **说明**  该参数从宜搭应用中获取。 ![[development-create-multiple-form-instances_O1CN01JtSC2M20A7ARy6uY9_!!6000000006808-2-tps-2326-866.png]] |
| keepRunningAfterException | Boolean | 否 | 批量保存多条表单实例数据发生异常时是否跳过异常的表单实例并继续保存下一个表单实例数据。   * true：跳过 * false：不跳过   **说明**  当noExecuteExpression为false时此参数才生效。 |
