...
tags:
  ...
  - 服务端API
---
![[development-api-searchformdatas-v2_O1CN01Zx4PED22rkUkmRe1L_!!6000000007174-2-tps-226-40.png]]![[development-api-searchformdatas-v2_O1CN01UwOv1C27lrTuGFj2D_!!6000000007838-2-tps-17-30.png]]

更新于 2025-09-23调用本接口查询表单实例数据。

![[development-api-searchformdatas-v2_O1CN01nH6sKI1KANVwd40t6_!!6000000001123-2-tps-950-466.png]]
...
| 企业内部应用 | 支持 | 宜搭表单数据读权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=org&api=yida_2.0%23SearchFormDatas) |
| 第三方企业应用 | 支持 | 宜搭表单数据读权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=isv&api=yida_2.0%23SearchFormDatas) |
...
| --- | --- | --- | --- |
| 名称 | 类型 | 是否必填 | 描述 |
| appType | String | 是 | 应用编码。 ![[development-api-searchformdatas-v2_O1CN01auMafb1gfuQBoESlW_!!6000000004170-2-tps-2694-1114.png]] |
| systemToken | String | 是 | 应用密钥。在应用数据中获取。 ![[development-api-searchformdatas-v2_O1CN01jA8xVe1ZStrN1fmGj_!!6000000003194-2-tps-2698-1144.png]] |
| userId | String | 是 | 钉钉userId |
| language | String | 否 | 语言。取值：   * **zh\_CN**：中文（默认值） * **en\_US**：英文 |
| formUuid | String | 是 | 表单ID。 ![[development-api-searchformdatas-v2_O1CN01HdY3Dp1PuFLl6yIFS_!!6000000001900-2-tps-2688-1160.png]] |
| searchFieldJson | String | 否 | 根据表单内组件值查询，示例值如下：  ``` {     "textField_jcr0069m": "danhang",     "textareaField_jcr0069n": "duohang",     "numberField_jcr0069o": [         "1",         "10"     ],     "radioField_jcr0069p": "选项一",     "selectField_jcr0069q": "选项一",     "checkboxField_jcr0069r": [         "选项二"     ],     "multiSelectField_jcr0069s": [         "选项二",         "选项三"     ],     "dateField_jcr0069t": [         1514736000000,         1517414399000     ],     "cascadeDate_jcr0069u": [ ``` |
| currentPage | Integer | 否 | 分页参数，当前页。 |
| pageSize | Integer | 否 | 分页参数，每页显示条数。 |
| originatorId | String | 否 | 根据数据提交人工号查询。 |
| createFromTimeGMT | String | 否 | 查询创建数据列表的开始时间，格式：`yyyy-MM-dd`。  **说明**  **createFromTimeGMT**和**createToTimeGMT**两个时间构造一个时间段，查询在该时间段创建的数据列表。 |
| createToTimeGMT | String | 否 | 查询创建数据列表的结束时间，格式：`yyyy-MM-dd`。  **说明**  **createFromTimeGMT**和**createToTimeGMT**两个时间构造一个时间段，查询在该时间段创建的数据列表。 |
| modifiedFromTimeGMT | String | 否 | 查询修改数据列表的开始时间，格式：`yyyy-MM-dd`。  **说明**  **modifiedFromTimeGMT**和**modifiedTo**构成一个时间段，查询在该时间段有修改的数据列表。 |
| modifiedToTimeGMT | String | 否 | 查询修改数据列表的结束时间，格式：`yyyy-MM-dd`。  **说明**  **modifiedFromTimeGMT**和**modifiedTo**构成一个时间段，查询在该时间段有修改的数据列表。 |
...
| version | Long | 数据版本 |
| creatorUserId | String | 创建人 |
| modifierUserId | String | 修改人 |
| sequence | String | 批次号 |
