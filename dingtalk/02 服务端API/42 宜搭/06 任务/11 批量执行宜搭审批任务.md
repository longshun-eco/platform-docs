...
tags:
  ...
  - 服务端API
---
![[development-batch-execution-should-take-the-lead-of-approval-tasks_O1CN01Zx4PED22rkUkmRe1L_!!6000000007174-2-tps-226-40.png]]![[development-batch-execution-should-take-the-lead-of-approval-tasks_O1CN01UwOv1C27lrTuGFj2D_!!6000000007838-2-tps-17-30.png]]

更新于 2025-10-09调用本接口批量执行宜搭表单实例的审批任务。

例如，员工小钉在宜搭任务中心有3个待处理的宜搭审批单。如下图所示，调用本接口批量同意前2条审批单。

![[development-batch-execution-should-take-the-lead-of-approval-tasks_O1CN01vQM8rC1DqmtkmkVq4_!!6000000000268-2-tps-2592-1268.png]]

接口调用成功后，如下图所示，页面显示“操作成功”。

![[development-batch-execution-should-take-the-lead-of-approval-tasks_O1CN01IIERCr26m1eGpfzVn_!!6000000007703-2-tps-2754-1078.png]]

**说明**

* 批量审批操作目前支持的审批动作仅包含：同意、拒绝。
* 调用本接口之前，请先前往流程设计器中，确保指定的审批节点已经开启了批量审批的功能。

  ![[development-batch-execution-should-take-the-lead-of-approval-tasks_O1CN019lRzwl1bM9BIdJ5Fb_!!6000000003450-2-tps-2418-1072.png]]
...
| 企业内部应用 | 支持 | 宜搭任务写权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=org&api=yida_1.0%23ExecuteBatchTask) |
| 第三方企业应用 | 支持 | 宜搭任务写权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=isv&api=yida_1.0%23ExecuteBatchTask) |
| 第三方个人应用 | 暂不支持 | 宜搭任务写权限 | 暂不支持 |
...
| appType | String | 是 | 宜搭应用编码。  **说明**  该参数从宜搭应用中获取。 ![[development-batch-execution-should-take-the-lead-of-approval-tasks_O1CN01auMafb1gfuQBoESlW_!!6000000004170-2-tps-2694-1114.png]] |
| systemToken | String | 是 | 宜搭应用密钥。  **说明**  该参数从宜搭应用中获取。 ![[development-batch-execution-should-take-the-lead-of-approval-tasks_O1CN01jA8xVe1ZStrN1fmGj_!!6000000003194-2-tps-2698-1144.png]] |
| remark | String | 否 | 审批意见。 |
| userId | String | 是 | 操作人userId。 |
| taskInformationList | String | 是 | 批量执行的审批任务列表，数组对象格式，每个元素值包含taskId和formInstId两个子属性。  **说明**   * taskId，表示待执行的审批任务Id，调用[查询流程运行任务（VPC）](/document/development/query-process-running-tasks-vpc)接口获取。 * formInstId，表示当前taskId所属的宜搭表单实例Id，调用[获取实例ID列表](/document/development/api-getinstanceidlist-v2)接口或者[获取多个表单实例ID](/document/development/api-searchformdataidlist-v2)接口获取。   该参数示例值如下：  ``` "[{\"taskId\":\"2291xxx\",\"formInstId\":\"d84a79xxx\"}, {\"taskId\":\"2291xxx\",\"formInstId\":\"f8035e2axxx\"}]" ``` |
