...
tags:
  ...
  - 服务端API
---
![[development-the-query-should-be-based-on-the-execution-records-of_O1CN01Zx4PED22rkUkmRe1L_!!6000000007174-2-tps-226-40.png]]![[development-the-query-should-be-based-on-the-execution-records-of_O1CN01UwOv1C27lrTuGFj2D_!!6000000007838-2-tps-17-30.png]]

更新于 2025-10-09根据宜搭表单Id查询对应的服务调用执行记录信息，包括服务调用状态、服务调用地址和服务调用结果等。

例如，企业宜搭应用数据管理列表中，其中一个表单实例的服务调用记录如下图，调用本接口获取服务调用记录详情信息。

![[development-the-query-should-be-based-on-the-execution-records-of_O1CN01MhqNwr1N3unAwbTTR_!!6000000001515-2-tps-2414-1438.png]]
...
| 企业内部应用 | 支持 | 宜搭表单数据读权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=org&api=yida_1.0%23QueryServiceRecord) |
| 第三方企业应用 | 支持 | 宜搭表单数据读权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=isv&api=yida_1.0%23QueryServiceRecord) |
| 第三方个人应用 | 暂不支持 | 宜搭表单数据读权限 | 暂不支持 |

## 请求方法

```
GET /v1.0/yida/services/invocationRecords?userId=String&hookType=String&systemToken=String&hookUuid=String&sourceUuid=String&requestUrl=String&success=Boolean&pageNumber=Integer&instanceId=String&invokeAfterDateGMT=String&pageSize=Integer&invokeStatus=String&appType=String&invokeBeforeDateGMT=String&formUuid=String HTTP/1.1
...
| systemToken | String | 是 | 宜搭应用密钥。  **说明**  该参数从宜搭应用中查看。 ![[development-the-query-should-be-based-on-the-execution-records-of_O1CN01JtSC2M20A7ARy6uY9_!!6000000006808-2-tps-2326-866.png]] |
| hookUuid | String | 否 | 本次服务调用的唯一ID，自定义参数。 |
| sourceUuid | String | 否 | 被重试的服务调用唯一ID。  **说明**  该参数值传某个执行失败的调用服务的唯一ID。 |
| requestUrl | String | 否 | 服务调用地址中包含的部分字符串，用于模糊查询。 |
| success | Boolean | 否 | 服务调用是否成功。   * true：成功 * false：不成功   **说明**  不传该参数则查询全部。 |
| pageNumber | Integer | 否 | 当前页码，从1开始。 |
| instanceId | String | 是 | 宜搭表单实例Id，调用[获取实例ID列表](/document/development/api-getinstanceidlist-v2)接口或者[获取多个表单实例ID](/document/development/api-searchformdataidlist-v2)接口获取。 |
...
| appType | String | 是 | 宜搭应用编码。  **说明**  该参数从宜搭应用中查看。 ![[development-the-query-should-be-based-on-the-execution-records-of_O1CN01JtSC2M20A7ARy6uY9_!!6000000006808-2-tps-2326-866.png]] |
| invokeBeforeDateGMT | String | 否 | 查询调用服务的结束时间。 |
| formUuid | String | 是 | 宜搭表单编码。  **说明**  该参数从宜搭应用中查看。 ![[development-the-query-should-be-based-on-the-execution-records-of_O1CN01JtSC2M20A7ARy6uY9_!!6000000006808-2-tps-2326-866.png]] |
