---
title: "在钉钉客联互通群中使用钉内账号发送消息新版SDK"
...
tags:
  ...
  - 服务端API
---
![[development-send-b2c-messages_O1CN01Zx4PED22rkUkmRe1L_!!6000000007174-2-tps-226-40.png]]![[development-send-b2c-messages_O1CN01UwOv1C27lrTuGFj2D_!!6000000007838-2-tps-17-30.png]]

更新于 2025-09-24调用本接口，实现钉内账号给钉外账号或者互通群发送消息。
...
| receiverId | String | 否 | 钉外账号在业务系统内的唯一标志，调用[创建钉钉客联钉外账号](https://open.dingtalk.com/document/development/create-bc-account-association)接口获取，长度限制为1～64个字符，例如：1107\*\*\*\*2120。  **说明**  单聊场景必填，可实现钉内账号向钉外账号发送单聊消息。 |
| openConversationId | String | 否 | 群会话openConversationId，可调用[创建钉钉客联普通互通群](https://open.dingtalk.com/document/development/create-common-group-new-version) / [创建钉钉客联两人互通群](https://open.dingtalk.com/document/development/creating-two-groups-of-people)接口获取，长度限制为1～32个字符，例如：14da\*\*\*\*2760。  **说明**  群聊场景必填，可实现钉内账号向互通群内发送群聊信息。 |
| messageType | String | 是 | 消息类型，取值：   * **text**：文本类型 * **link**：链接类型 |
| message | String | 是 | 消息内容。  **说明**  请参考本文**消息格式说明**。 |
| code | String | 是 | 发送者在钉钉客联应用内的个人授权码，获取方式：https://login.dingtalk.com/oauth2/auth?redirect\_uri=https%3A%2F%2Fexample.org%2Fa%2Fb&response\_type=code&client\_id=suitezl\*\*\*pimsjn&scope=openid corpid&state=dddd&prompt=consent&corpId=ding3xxx   * **redirect\_uri**地址传企业目标页面地址，必须UrlEncode处理。 * **response\_type**为固定值code。 * **client\_id**为固定值，是钉钉客联应用的suiteKey。 * **scope**可以固定为openid corpid。（中间有空格） * **corpId**传发送者所在企业的corpId值。   **说明**  \*\* 每发送一条消息后，都需要重新获取一个新的授权码\*\*。 |
