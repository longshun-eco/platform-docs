...
tags:
  ...
  - 服务端API
---
![[development-initiate-the-approval-process_O1CN01Zx4PED22rkUkmRe1L_!!6000000007174-2-tps-226-40.png]]![[development-initiate-the-approval-process_O1CN01UwOv1C27lrTuGFj2D_!!6000000007838-2-tps-17-30.png]]

更新于 2025-09-08调用本接口发起宜搭审批流程到钉钉开放平台。

**重要**

为提升接口使用体验，针对宜搭部分接口进行升级，本文档预计于 2024 年 11 月 1 日迁移至历史文档（不推荐）目录，接口不再支持新应用接入, 已接入的应用可继续调用，且本接口仅保持现有功能，不再新增支持其他能力。

* 如果未使用本接口，推荐使用[发起宜搭审批流程](/document/orgapp/api-startinstance-v2)接口。
* 如果已使用本接口，建议您根据自身实际情况评估是否切换至推荐接口。

![[development-initiate-the-approval-process_O1CN01rLyTMf1NUTk9O6HZd_!!6000000001573-2-tps-960-288.png]]
...
| --- | --- | --- | --- |
| 名称 | 类型 | 是否必填 | 描述 |
| appType | String | 是 | 应用编码。 ![[development-initiate-the-approval-process_O1CN01auMafb1gfuQBoESlW_!!6000000004170-2-tps-2694-1114.png]] |
| systemToken | String | 是 | 应用秘钥。在应用数据中获取。 ![[development-initiate-the-approval-process_O1CN01jA8xVe1ZStrN1fmGj_!!6000000003194-2-tps-2698-1144.png]] |
| userId | String | 是 | 用户的userid。 |
| language | String | 否 | 语言，取值：   * **zh\_CN**：中文（默认值） * **en\_US**：英文 |
| formUuid | String | 是 | 表单唯一编码。 ![[development-initiate-the-approval-process_O1CN01HdY3Dp1PuFLl6yIFS_!!6000000001900-2-tps-2688-1160.png]] |
| formDataJson | String | 是 | 表单数据，示例： `"{\"textField_jcpm6agt\": \"单行\",\"employeeField_jcos0sar\": [\"workno\"]}"`   * key：流程组件标识，宜搭表单编辑页面，高级设置中查看。 * value：流程组件内的值。 ![[development-initiate-the-approval-process_O1CN01yTqqI51eViHNpsmb2_!!6000000003877-2-tps-2662-1014.png]] |
| processCode | String | 否 | 流程编码。 ![[development-initiate-the-approval-process_O1CN01wfayc31oU13xpzPfX_!!6000000005227-2-tps-2704-1152.png]] |
| departmentId | String | 否 | 发起人所在部门ID。 |
| processData | String | 否 | 发起人自选节点所选的审批人信息。  **说明**   * processData 参数是一个 json 数组，元素个数代表发起人节点个数； * 对于每个发起人自选节点，key 代表发起人自选节点唯一标识，value 代表需要指定的审批人 userId 列表，为数组格式，可以是一个或者多个，以英文逗号隔开。 * 发起人自选节点唯一标识可以从[预览审批流程](https://open.dingtalk.com/document/orgapp/api-previewpublishedprocess)接口的返回值中的 operator 字段中获取。预览含有发起人自选节点的审批流程时，发起人自选节点的 operator 字段会返回一个前缀为\*\*\_*optionalApproval***的唯一标识，例如：**\_\_optionalApproval\_node\_ocltdztr2b3\*\*。 |
