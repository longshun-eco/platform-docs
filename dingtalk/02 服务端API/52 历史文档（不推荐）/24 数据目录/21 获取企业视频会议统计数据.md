---
title: "获取企业视频会议统计数据新版SDK"
source: "https://open.dingtalk.com/document/development/get-enterprise-video-conference-statistics"
...
tags:
  ...
  - 服务端API
---
![[development-get-enterprise-video-conference-statistics_O1CN01Zx4PED22rkUkmRe1L_!!6000000007174-2-tps-226-40.png]]![[development-get-enterprise-video-conference-statistics_O1CN01UwOv1C27lrTuGFj2D_!!6000000007838-2-tps-17-30.png]]

更新于 2025-09-08调用本接口获取企业视频会议发起和参与情况的统计数据。
...
调用本接口前需完成接口权限申请，详情请参考[权限申请](https://open.dingtalk.com/document/orgapp/add-api-permission)和[开发须知](https://open.dingtalk.com/document/orgapp/before-you-start)。

|  |  |  |  |  |  |  |  |  |
| --- | --- | --- | --- | --- | --- | --- | --- | --- |
| 统计日期 | 组织ID | 视频会议发起次数 | 视频会议发起用户数 | 视频会议成功发起次数 | 视频会议参与用户数 | 视频会议成功参与用户数 | 视频会议参与人次 | 视频会议时长（分钟）｜ |
| 20210101 | 12345 | 3 | 3 | 3 | 65 | 60 | 69 | 58.82 |
...
| 企业内部应用 | 支持 | 数据服务目录在线会议统计读权限 | **重要**  暂不支持新增申请。 |
| 第三方企业应用 | 暂不支持 | 数据服务目录在线会议统计读权限 | 暂不支持 |
| 第三方个人应用 | 暂不支持 | 数据服务目录在线会议统计读权限 | 暂不支持 |

## 请求方法

```
GET /v1.0/datacenter/vedioMeetingData?statDate=String HTTP/1.1
...
```
GET /v1.0/datacenter/vedioMeetingData?statDate=20210620 HTTP/1.1
Host:api.dingtalk.com
...

{
  "metaList" : [ {
    ...
    "period" : "DAY",
    "kpiId" : "4049",
    "kpiCaliber" : "企业内发起并成功接通的视频会议总时长，单位分钟。",
    "kpiName" : "视频会议时长"
  }, {
    "unit" : "人次",
    "period" : "DAY",
    "kpiId" : "4050",
    "kpiCaliber" : "企业内用户被邀请参与视频会议的人次，包括接通及未接通的情况。",
    "kpiName" : "视频会议参与人次"
  }, {
    "unit" : "次",
    "period" : "DAY",
    "kpiId" : "4051",
    "kpiCaliber" : "视频会议发起次数，包含接通及未接通的情况。",
```
