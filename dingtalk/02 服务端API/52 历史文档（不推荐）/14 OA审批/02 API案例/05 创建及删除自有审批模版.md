---
title: "创建及删除自有审批模版"
source: "https://open.dingtalk.com/document/development/self-built-workflow-initiation-and-completion-of-the-approval-process"
...
tags:
  ...
  - 服务端API
---
![[development-self-built-workflow-initiation-and-completion-of-the-approval-process_O1CN01Zx4PED22rkUkmRe1L_!!6000000007174-2-tps-226-40.png]]![[development-self-built-workflow-initiation-and-completion-of-the-approval-process_O1CN01UwOv1C27lrTuGFj2D_!!6000000007838-2-tps-17-30.png]]

更新于 2025-09-08本案例介绍了自有OA审批，如何通过API发起自有OA审批等流程。
...
本文档展示了，创建一个企业内部应用，使用**自有**OA审批提供的API，实现创建审批模版、发起审批实例、完成审批、删除模版等流程：
...
3. 根据审批实例`process_instance_id`，调用服务端API-[更新实例状态](/document/orgapp/to-do-instance-status)接口，实现审批实例状态的更新。
4. 若需要删除自有审批模板，根据审批模板`process_code`，可以调用服务端API-[删除模板](/document/orgapp/delete-a-template)接口，实现删除自有审批模板。

   **说明**
   * 企业内部应用，只删除模版，不删除通过模版创建的实例和待办任务。
   * 第三方企业应用，删除模版，同时删除使用该模版创建的实例和待办任务。
...
1. 登录[开发者后台](https://open-dev.dingtalk.com/#/)，创建[企业内部应用](/document/orgapp/create-orgapp)。
   ...
   * 开发方式：选择企业自主开发。
     ...
     ![[development-self-built-workflow-initiation-and-completion-of-the-approval-process_p477411.png]]

## 步骤二：获取AppKey和AppSecret

获取应用AppKey和AppSecret信息。

![[development-self-built-workflow-initiation-and-completion-of-the-approval-process_p477413.png]]

## 步骤三：添加接口权限

[添加接口调用权限](/document/orgapp/permission-application-and-basic-concepts)。单击“OA审批”，申请OA审批对应的权限。

![[development-self-built-workflow-initiation-and-completion-of-the-approval-process_p477417.png]]
...
4. 若需要删除自有审批模板，根据审批模板`process_code`，可以调用服务端API-[删除模板](/document/orgapp/delete-a-template)接口，实现删除自有审批模板。
   ...
   * 第三方企业应用，删除模版，同时删除使用该模版创建的实例和待办任务。
   ...
   ```java
    public void processDelete() throws ApiException {
