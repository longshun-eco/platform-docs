...
tags:
  ...
  - 服务端API
---
![[development-move-a-file-or-folder_O1CN01Zx4PED22rkUkmRe1L_!!6000000007174-2-tps-226-40.png]]![[development-move-a-file-or-folder_O1CN01UwOv1C27lrTuGFj2D_!!6000000007838-2-tps-17-30.png]]

更新于 2025-09-12调用本接口，移动文件或文件夹的位置。
...
| 企业内部应用 | 支持 | 企业存储文件写权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=org&api=storage_1.0%23MoveDentry) |
| 第三方企业应用 | 支持 | 企业存储文件写权限 | [API Explorer](https://open-dev.dingtalk.com/apiExplorer#/?devType=isv&api=storage_1.0%23MoveDentry) |
| 第三方个人应用 | 暂不支持 | 企业存储文件写权限 | 暂不支持 |

## 请求方法

```
POST /v1.0/storage/spaces/{spaceId}/dentries/{dentryId}/move?unionId=String HTTP/1.1
...
| dentryId | String | 是 | 需要被移动的文件或文件夹Id，调用[获取文件或文件夹列表](/document/development/obtain-the-file-list-storage)接口获取id参数值。 |
...
| --- | --- | --- | --- |
| 名称 | 类型 | 是否必填 | 描述 |
| targetSpaceId | String | 是 | 需要存放的目标空间Id，调用[添加空间](/document/development/add-space)接口获取。 |
| targetFolderId | String | 是 | 需要存放的位置父目录Id，调用[获取文件或文件夹列表](/document/development/obtain-the-file-list-storage)接口获取id参数值。根目录时，该参数是0。 |
